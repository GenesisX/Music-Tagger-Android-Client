...
/**
 * A fragment representing a single MP3 File detail screen. This fragment is
 * either contained in a {@link MPFileListActivity} in two-pane mode (on
 * tablets) or a {@link MPFileDetailActivity} on handsets.
 */
public class MPFileDetailFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View root = inflater.inflate(R.layout.fragment_mpfile_detail,
				container, false);
		...
		mAlbumArtistName = (TextView) root
		...
		mPrev.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				/*
				 * if (MusicUtils.mService == null) return; try { if
				 * (MusicUtils.mService.position() < 2000) {
				 * MusicUtils.mService.prev(); } else {
				 * MusicUtils.mService.seek(0); MusicUtils.mService.play(); } }
		...
		mNext.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				/*
				 * if (MusicUtils.mService == null) return; try {
				 * MusicUtils.mService.next(); } catch (RemoteException ex) {
				 * ex.printStackTrace(); }
				 */
			}
		});
		...
		if (mItem != null) {
			((TextView) root.findViewById(R.id.mpfile_detail)).setText(mItem
					.getParent());
